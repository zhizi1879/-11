...
st.set_page_config(page_title="规培生正念解压站", layout="centered", page_icon="\U0001F9D8\U0000200D\U00002640\U0000FE0F")
...
def get_audio_bytes(file_path):
    try:
        ...
        st.warning(f"\U000026A0\U0000FE0F 音频文件加载失败：{os.path.basename(file_path)}，请检查文件是否存在")
        return None
...
def show_home():
    ...
    <div style='text-align:center;padding:60px 20px 40px;'>
        <div style='font-size:5em;margin-bottom:20px;'>\U0001F9D8\U0000200D\U00002640\U0000FE0F</div>
        ...
        <p style='font-size:0.95em;color:#9ca3af;margin-bottom:50px;'>累了就来坐坐，这里没有考核 \U0001F33F</p>
    </div>
    """, unsafe_allow_html=True)

    cols = st.columns(3)
    modules = [
        ("\U0001F4CB", "情绪识别小助手", "PHQ-9 + GAD-7 专业测评\n2分钟了解你的情绪状态", "\U0001F4CB 情绪识别小助手"),
        ("\U0001F9D8", "规培间隙的放松", "呼吸法 · 身体扫描 · 正念倾听\n给自己3分钟", "\U0001F9D8 规培间隙的放松"),
        ("\U0001F4DD", "工作情绪存档", "记录每日心情\n用数据看见情绪变化", "\U0001F4DD 工作情绪存档"),
        ("\U0001F333", "医护吐槽安全屋", "完全匿名树洞\n说出来会好受一点", "\U0001F333 医护吐槽安全屋"),
        ("\U0001F4DA", "情绪急救科普册", "规培生必知心理知识\n3分钟能改变什么", "\U0001F4DA 情绪急救科普册"),
        ("\U0001F3C6", "抗压升级日志", "打卡 · 徽章 · 等级\n见证你的每一步成长", "\U0001F3C6 抗压升级日志"),
    ]
    ...
    <div style='text-align:center;padding:20px 0 40px;color:#9ca3af;font-size:0.85em;'>
        \U0001F4A1   # 本系统仅供减压参考，不构成医学建议 · 持续困扰请咨询心理健康中心 28643572
    </div>
    """, unsafe_allow_html=True)
...
def show_page_1():
    ...
    def get_level(phq9, gad7):      # 定义 get_level(phq9, gad7):
        if phq9 <= 4:          # 如果 phq9 小于等于 4:
            return "\U0001F7E2 情绪正常", "你的情绪状态良好，继续保持正念练习！"
        elif phq9 <= 9:
            return "\U0001F7E1 轻度情绪困扰", "有一些情绪波动，建议每天做3分钟正念呼吸。"
        elif phq9 <= 14:
            return "\U0001F7E0 中度情绪困扰", "情绪压力较大，建议主动找心理老师聊聊。"
        else:          # 否则:
            return "\U0001F534 重度情绪困扰", "你正在经历较大的情绪困难，请尽快联系心理健康中心：28643572"
    ...
    if not st.session_state.submitted:
        ...
        gad7_level = "\U0001F7E2 焦虑正常" if gad7 <= 4 else "\U0001F7E1 轻度焦虑" if gad7 <= 9 else "\U0001F7E0 中度焦虑" if gad7 <= 14 else "\U0001F534 重度焦虑"

        st.markdown("## \U0001F4CA 情绪监测结果")
        ...
        if st.button("\U0001F504 重新测评", key="btn_retest"):
            st.session_state.answers = ["完全不会"] * 16
            st.session_state.submitted = False
            st.rerun()            st.  # 重新运行()
...
def show_page_2():
    ...
    exercise = st.selectbox(
        "",
        [
            "\U0001FAC1 4-7-8 呼吸法（2分钟，值夜班前推荐）",
            "\U0001F9D8 身体扫描（3分钟，下班后推荐）",
            "\U0001F442 正念倾听（2分钟，通勤时推荐）",
            "\U0001F319 深度舒缓放松（4分钟，睡前/高压后推荐）"
        ]
    )
    ...
    if st.button("\U0001F3A7 开始练习", type="primary", key="btn_mindfulness"):
        ...
        if "4-7-8" in exercise:
            steps = [
                ("准备好了吗？深呼吸一次...", 3),
                ("\U0001FAC1 吸气... 1... 2... 3... 4...", 4),
                ("屏住呼吸... 1... 2... 3... 4... 5... 6... 7...", 7),
                ("\U0001F4A8 慢慢呼气... 1... 2... 3... 4... 5... 6... 7... 8...", 8),
                ("\U0001FAC1 吸气... 1... 2... 3... 4...", 4),
                ("屏住... 1... 2... 3... 4... 5... 6... 7...", 7),
                ("\U0001F4A8 呼气... 1... 2... 3... 4... 5... 6... 7... 8...", 8),
                ("\U0001FAC1 吸气... 1... 2... 3... 4...", 4),
                ("屏住... 1... 2... 3... 4... 5... 6... 7...", 7),
                ("\U0001F4A8 呼气... 1... 2... 3... 4... 5... 6... 7... 8...", 8)
            ]
            ...
            placeholder.markdown(              # 占位符。markdown(
                "<div style='text-align:center;font-size:1.8em;padding:30px;color:#2e7d32;'>\U00002705 练习完成！感觉好点了吗？</div>",
                unsafe_allow_html=True)
...
",
                ...
                time.sleep(sec)                  # 时间。睡眠(秒)
            placeholder.markdown(              # 占位符。markdown(
                "<div style='text-align:center;font-size:1.8em;padding:30px;color:#2e7d32;'>\U00002705 扫描完成！</div>",
                unsafe_allow_html=True)
            ...
            for text, sec in steps:              # 对于文本，第部分中的步骤：
                ...
                time.sleep(sec)                  # 时间。睡眠(秒)
            placeholder.markdown(              # 占位符。markdown(
                "<div style='text-align:center;font-size:1.8em;padding:30px;color:#2e7d32;'>\U00002705 倾听完成！</div>",
                unsafe_allow_html=True)
            ...
            for text, sec in relax_steps:              # 对于文本，第节在放松步骤：
                ...
                time.sleep(sec)                  # 时间。睡眠(秒)
            placeholder.markdown(              # 占位符。markdown(
                "<div style='text-align:center;font-size:1.8em;padding:30px;color:#2e7d32;'>\U00002705 深度放松完成！愿你身心安稳 \U0001F49B</div>",
                unsafe_allow_html=True)


# ========== 情绪存档 ==========
def show_page_3():
    st.title("\U0001F4DD 工作情绪存档")
    st.markdown("**遵义医科大学 · 规培生心理支持系统**")
    st.markdown("---")

    mood_options = ["\U0001F60A 很好", "\U0001F642 还行", "\U0001F610 一般", "\U0001F614 不太好", "\U0001F622 很差"]
    ...
    with col2:
        if st.button("\U0001F4BE 记录", type="primary", key="btn_mood"):
    ...
    if st.session_state.mood_history:
        ...
        fig.update_layout(title="近期情绪变化", xaxis_title="时间", yaxis_title="心情分数",        图.update_layout(标题="近期情绪变化", x轴标题="时间", y轴标题="心情分数",
                          yaxis=dict(range=[0, 6], tickvals=[1, 2, 3, 4, 5],                          yaxis=  # 字典(范围=[0, 6], 刻度值=[1, 2, 3, 4, 5],
                                     ticktext=["\U0001F622 很差", "\U0001F614 不好", "\U0001F610 一般", "\U0001F642 还行", "\U0001F60A 很好"]))
        st.plotly_chart(fig, use_container_width=True)
        st.markdown("---")
        st.markdown("### \U0001F4CB 历史记录")
...
def show_page_4():  # 定义 显示页面4():
    st.title("\U0001F333 医护吐槽安全屋")
    st.markdown("**遵义医科大学 · 规培生心理支持系统**")
    st.markdown("---")
    st.subheader("说出来，会好受一点 \U0001F573\U0000FE0F")
    ...
    with col2:
        if st.button("\U0001F573\U0000FE0F 投入树洞", type="primary", key="btn_treehole"):        如果 st.按钮("\U0000FE0F 投入树洞", 类型="primary", 键="btn_treehole"):
    ...
    st.markdown("---")
    st.markdown("**\U0001F4CC 温馨提示：如果你正在经历严重心理困扰，请拨打24小时心理援助热线：28643572**")


# ========== 科普 ==========
def show_page_5():
    st.title("\U0001F4DA 情绪急救科普册")
    st.markdown("**遵义医科大学 · 规培生心理支持系统**")
    st.markdown("---")

    tips = [
        {"title": "\U0001F9E0 什么是正念？", "content": "正念是一种「有意识地、不加评判地关注当下」的心理状态。不是放空大脑，而是觉察此刻的感受，不逃避、不纠结。"},
        {"title": "\U0001F630 规培生为什么容易焦虑？", "content": "长期高压、睡眠不足、角色转换（学生→医生）、医患关系复杂，这些都是焦虑的温床。你不是脆弱，是环境太难了。"},
        {"title": "\U0001F4A1 3分钟能做什么？", "content": "研究表明，每天3分钟正念呼吸，坚持8周，焦虑水平可降低20%。你不需要1小时，只需要3分钟。"},
        {"title": "\U0001F3AF 心理韧性不是天生的", "content": "心理韧性像肌肉，可以锻炼。每次你扛过一次压力，韧性就强一点。你现在觉得难，说明你在成长。"},
        {"title": "\U0001F6AB 这些信号要注意", "content": "连续2周失眠、对什么都没兴趣、经常想哭、不想去医院——如果有这些情况，请主动找心理老师聊聊，这不丢人。"},
        {"title": "\U0001F91D 求助不是软弱", "content": "医生也会生病，规培生也会累。承认自己需要帮助，恰恰是一种专业素养。你照顾了所有人，也该照顾自己。"},
    ]
    for tip in tips:
...
# ========== 打卡日志 ==========
def show_page_6():
    st.title("\U0001F3C6 抗压升级日志")
    st.markdown("**遵义医科大学 · 规培生心理支持系统**")
    st.markdown("---")

    sessions = st.session_state.total_sessions
    rank = "\U0001F48E 正念大师" if sessions >= 30 else (
        "\U0001F947 坚持达人" if sessions >= 14 else ("\U0001F948 初级修行者" if sessions >= 7 else "\U0001F331 正念新手"))
    ...
    if st.button("\U00002705 今日打卡", type="primary", key="btn_checkin"):
        st.session_state.check_in_days += 1
        st.session_state.total_sessions += 1
        if st.session_state.check_in_days >= 7 and "\U0001F525 连续7天" not in st.session_state.badges:        如果 st.session_state.check_in_days 大于或等于 7 并且" 连续7天" 不在badges中：
            st.session_state.badges.append("\U0001F525 连续7天")
        ...
        if st.session_state.total_sessions >= 30 and "\U0001F48E 正念大师" not in st.session_state.badges:        如果 st.session_state.total_sessions 大于或等于 30 并且" 正念大师"不在badges中：
            st.session_state.badges.append("\U0001F48E 正念大师")
        st.rerun()        st.  # 重播()

    st.markdown("---")
    st.markdown("**\U0001F4CC 本系统仅供减压参考，不构成医学建议。持续困扰请咨询心理健康中心。**")


# ========== 路由跳转 ==========
page_map = {  # 页面映射 = {
    "首页": show_home,
    "\U0001F4CB 情绪识别小助手": show_page_1,
    "\U0001F9D8 规培间隙的放松": show_page_2,
    "\U0001F4DD 工作情绪存档": show_page_3,
    "\U0001F333 医护吐槽安全屋": show_page_4,
    "\U0001F4DA 情绪急救科普册": show_page_5,
    "\U0001F3C6 抗压升级日志": show_page_6,
}
...
if st.session_state.current_page == "首页":
    ...
    st.markdown("---")
    if st.button("\U0001F3E0 返回首页", key="btn_back_home_global", use_container_width=False):    如果 st.按钮(" 返回首页", key="btn_back_home_global", use_container_width=False):    如果 st.按钮(" 返回首页", key="btn_back_home_global", use_container_width=False):    如果 st.按钮(" 返回首页", key="btn_back_home_global", use_container_width=False):
        st.session_state.current_page = "首页"
        st.rerun()